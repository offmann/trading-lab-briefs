...
| File | Job | Answers |
|------|-----|---------|
| `index.html` | **Investor (home / default)** | Plain English: what was tested that matters, last updated to the second, what it means for an investor, and “$1,000 → $X vs just hold over a clear timeframe.” Lead with the two big numbers when they still hold: **Just hold Bitcoin $19,879** · **ALGO lab pick 20/12 $18,662**. Rows say **ALGO breakout rule 20/12** / **ETH breakout rule 10/5** (keep the knobs; do not say Donchian on Investor). Optional one-line legend: “Just hold = own the coin. Lab pick = best paper rule we found so far.” |
| `leaderboard.html` | Scoreboard + plot | Ranked paper strategies by sleeve (**BTC / ALGO / ETH**) vs just hold, Lab pick / Still testing / Dropped. Plus a lollipop: each point = an experiment; X = paper end $ from $1,000; Y = ranked strategy. Sleeve filters; do not mix incompatible windows on one series. **Phone (~390px):** sleeve rows stack — strategy + verdict chip on the first line, end $ + vs hold on the second. Verdict must be visible without horizontal swipe. |
...
Hub pages use a **three-tab** nav: **Investor / Leaderboard / Experiments**. One row on a phone (~390px). Internal hub links carry `?v=20260912c`. Hub `<head>` includes `Cache-Control: no-cache` and a visible `build 20260912c` footer stamp. Rebuild injects `#gallery-glossary` JSON from `PUBLIC_VERDICT` so the Leaderboard plot reads the same labels. Rebuild also injects `gallery:phone-board` CSS so Leaderboard sleeve verdicts sit on the strategy line at ≤479px.

## Copy rules (hubs)

- Shorten aggressively. Clarity > completeness. No jargon walls.
- **Investor rows:** `Donchian N/M` → **breakout rule N/M** (rebuild translates this once). Keep 20/12 and 10/5 visible.
- **Chips:** say **Dropped**, never “Dropped vs hold”.
- **Investor footnote:** do not say “rolling out-of-sample test”. One plain line: the rule failed checks on later time windows.
...
- **Does not** overwrite Investor prose, leaderboard rank tables, or plot point data — except the jargon polish in `polish_hub_page` (Investor Donchian → breakout rule, Dropped vs hold → Dropped, later-window check line) and Leaderboard `gallery:phone-board` CSS (stacked phone rows). Those stay editorial otherwise.
