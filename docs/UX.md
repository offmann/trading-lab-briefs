...
Hub pages use a **three-tab** nav: **Investor / Leaderboard / Experiments**. One row on a phone (~390px). Internal hub links carry `?v=20260912b`. Hub `<head>` includes `Cache-Control: no-cache` and a visible `build 20260912b` footer stamp. Rebuild injects `#gallery-glossary` JSON from `PUBLIC_VERDICT` so the Leaderboard plot reads the same labels.
...
- **Does not** overwrite Investor prose, leaderboard rank tables, or plot point data — except the jargon polish in `polish_hub_page` (Investor Donchian → breakout rule, Dropped vs hold → Dropped, later-window check line). Those stay editorial otherwise.
